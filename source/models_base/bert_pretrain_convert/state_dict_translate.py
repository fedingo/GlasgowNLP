...
def BERT_self_attention_combiner(bert_state_dict, target_state_dict, layer):
    with open("source/models_base/bert_pretrain_convert/self_attention_map.json", "r") as f:
        key_map = json.load(f)
...
def translate_one_layer(bert_state_dict, target_state_dict, layer):
    with open("source/models_base/bert_pretrain_convert/layer_map.json", "r") as f:
        key_map = json.load(f)
...
def translate_from_hugginface_to_torch_BERT(bert_state_dict, num_layers = 12):
    ...
    with open("source/models_base/bert_pretrain_convert/extras_map.json", "r") as f:
        key_map = json.load(f)
